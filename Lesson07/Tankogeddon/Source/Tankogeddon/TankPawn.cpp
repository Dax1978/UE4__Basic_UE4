...
void ATankPawn::OnConstruction(const FTransform& Transform)
{
	...
	// когда танк будет заспавнен мы увидим его id
	GEngine->AddOnScreenDebugMessage(-1, 1, FColor::Red, FString::Printf(TEXT("Tank spawned %d"), SpawnId));
	UE_LOG(LogTemp, Warning, TEXT("Tank spawned %d"), SpawnId);
}
...
void ATankPawn::Shoot(int ShootType)
{
	...
	if (Cannon)
	{
		...
		// ѕо идее запас снар€дов должен убывать
		// Ќо надо разделить танки, т.к. на вражеском запас снар€дов убывает слишком уж быстро :)
		//ATankPawn::AmmoCount--;
...
void ATankPawn::BeginPlay()
{
	...
	// ¬ начале ставим пушку по умолчанию
	SetupCannon(CannonClass);
}

void ATankPawn::SetupCannon(TSubclassOf<ACannon1> InCannonClass)
{
	// ≈сли пушка была то старую уничтожаем
	if (Cannon)
	{
		Cannon->Destroy();
		Cannon = nullptr;
...
{
	// јктивируем эффекты при попадании
...
{
	// јктивируем эффекты перед уничтожением
...
void ATankPawn::Destroyed() {
	if (Cannon)
	{
		...
		// убираем это, т.к. при переходе на следующий уровень уничтожаютс€ все объекты
		// здесь срабатывает данный метод и возвращает нас на поле...
...
void ATankPawn::OnTargetRangeBeginOverlap(UPrimitiveComponent* OverlappedComp, AActor* Other, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	auto OtherTank = Cast<ATankPawn>(Other);
	if (OtherTank) 
	{
		// Targets.Add(Other);
